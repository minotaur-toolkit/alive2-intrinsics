...
public:
  ...
  static std::array<llvm::Intrinsic::ID, IR::X86IntrinBinOp::numOfX86Intrinsics>
      intrin_id;
};
...
template <binOp op> constexpr int bitSizeOp0() {
...
template <binOp op> constexpr int bitSizeOp1() {
  constexpr int lanes = return_shape_op1<op>().first;
  constexpr int bitwidth = return_shape_op1<op>().second;
  constexpr int bitSize = lanes * bitwidth;

  static_assert((bitSize == 32) || (bitSize == 128) || (bitSize == 256) ||
                (bitSize == 512));
  return bitSize;
}

template <binOp op> constexpr int bitSizeRet() {
...
template <binOp op> constexpr int bitwidthOp0() {
  constexpr int bitwidth = return_shape_op0<op>().second;

  static_assert((bitwidth == 8) || (bitwidth == 16) || (bitwidth == 32) ||
                (bitwidth == 64));
  return bitwidth;
}

template <binOp op> constexpr int bitwidthOp1() {
  constexpr int bitwidth = return_shape_op1<op>().second;
...
template <binOp op> constexpr int bitwidthRet() {
  constexpr int bitwidth = return_shape_ret<op>().second;
