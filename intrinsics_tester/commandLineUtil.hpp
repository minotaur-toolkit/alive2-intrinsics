...
struct CommandLineUtil
{
  ...
  CommandLineUtil(int argc, char** argv)
  {
    if(argc < 2)
    {
      std::cerr << "ERROR: Needs atleast one argument. Add -h for help." << std::endl;
      exit(1);
    }
...
private:
  ...
  void setRangeOfFunctionsToTest()
  {
    ...
    {
      if(arguments.at(i).at(0) == '-' && arguments.at(i).at(1) == 'r' && arguments.at(i).at(2) == '=')
      {
        indexOfArg = i;
	break;
      }
    ...
    {
      std::cerr << "Intrinsic range unable to be parsed, no '-' found." << std::endl;
      exit(1);
    }
    ...
    {
      std::cerr << "ERROR: Upper bound can not be lower or equal to the lower bound." << std::endl;
      exit(1);
    }
    else if (upperExclusiveBound > IR::X86IntrinBinOp::numOfX86Intrinsics)
    {
      std::cerr << "ERROR: Upper bound can not be higher than total number of intrinsics." << std::endl;
      exit(1);
    }
  }
  ...
  {
    std::cout <<	"This program runs random tests on the intrinsic semantics\n" <<
			...
			"Number of intrinsics available for testing: " << IR::X86IntrinBinOp::numOfX86Intrinsics <<
			"\n\nFlags:\n\n" <<
			"  -d\t\tDebug mode: prints the LLVM IR of all the function\n\t\tintrinsics, and counts how many duplicate (non-interesting)\n\t\tresults occurred. Disables progres bar.\n\n" <<
			"  -r=x-y\tSets a range of the intrinsics to test,\n\t\tuseful for scripting. Usage: -r=40-50 (tests intrinsics\n\t\tbetween 40 inclusive and 50 exclusive)\n\n" <<
			"  -h\t\tHelp information\n" <<
			std::endl;
  }
};
...
unsigned CommandLineUtil::upperExclusiveBound = IR::X86IntrinBinOp::numOfX86Intrinsics;
